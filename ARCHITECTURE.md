...
The long half of this repo's documentation, for agents and humans both.
`README.md` is the front door; this file is the map. `AGENTS.md` is the rule
book for anyone changing the code, and where the two overlap, AGENTS.md wins.

## What it is, and where it came from

MyCalMind is CalMind with the server taken out. It has been its own repo since
2026-08-22 — extracted from
[chere005/CalMind](https://github.com/chere005/CalMind) with its history, where
it lived as `CalMind-Local`, and renamed.

It is not a rewrite and not a lookalike. It is the app from CalMind's
`apps/app` and `packages/core`, copied whole and then gutted of everything that
needed a server. Every screen, every gesture, every rule is the one upstream
ships, because it is literally the same code.

The rename is display-deep only: the bundle ids, App Group, Bonjour service and
every persisted key keep their `calmindlocal` spellings, because those ARE the
installed app's identity and its data. `AGENTS.md` has the full list, and the
standing rule that none of them may move.
...
`AGENTS.md` goes a layer deeper on the code itself — the core/app split, the
record shape, the store seam, the routerless navigation, the theming, and the
three Expo modules that make up the native edge.
...
All of them entirely local — no server, no accounts; devices mirror each other
over Bonjour on the local network.

- **iOS and watchOS** — the SwiftUI watch app, the watch-face complication and
  the home-screen widget all ship inside the iOS build.
- **macOS** — a real Mac Catalyst app, installed at `/Applications/MyCalMind.app`.
- **Android** — builds, installs and launches on the local emulator.
- **Web — none, deliberately.** No server means nothing to build a web instance
  against.

`AGENTS.md`'s **Platforms** section carries the build lore each of these cost:
the Catalyst config plugin and its arm64-only limit, the
`ReactNativeDependencies` codesign repair that
`tools/patch-rndeps-catalyst.js` automates, and the watch target's product name.
...
For the Mac, iOS and Android builds by hand, use the same script the release
lane does — `sh tools/build-platforms.sh --mac` (Catalyst into `/Applications`),
`--android` (emulator), `--ios` (the iOS build and its installs) — naming none
of them means all three — and `--dry-run` to see the plan before anything runs.

`AGENTS.md` lists the rest of the commands, including how to run one core test
file by name without losing the `TZ=America/Chicago` the fixtures need.

## Releasing it

```sh
npm run dtp      # gates, bump, the Mac Catalyst app into /Applications, tag,
                 # push, then Android on the emulator and the iOS build+install
...
leaves the version untagged, so a re-run reuses it. Android and iOS come after
the push and are reported rather than fatal; the release has already happened
by then.

Since 2026-09-21 a release **does** reach the phone: the lane builds iOS Release
(watch companion included) and installs it, like every other app in the suite.
`tools/deploy-device.sh` (`npm run deploy:device`) stays as the explicit one-off
install, for doing it without shipping a release; it refuses to guess when no
single connected iPhone is found.

It did not always work that way, and the reason is kept because it expired
rather than being wrong: Apple's FREE developer team caps one physical device at
3 installed apps, the phone's three were CalMind, ChefMind and AcctMind, and
MyCalMind was deliberately not one of them. The team is paid now, so there is
nothing to ration. `AGENTS.md` has the full account, including the tell that the
rule had already drifted — the app was sitting on his phone at 1.8.0 the whole
time the docs said it was never installed.
...
- `AGENTS.md` — the standing rules, the code map, the platform build lore, and
  the list of traps that have already cost real time here. Anyone changing code
  reads it first.
- `~/GIT/AgentSuite/AGENTS.md` — the suite-wide baseline `AGENTS.md` imports.
